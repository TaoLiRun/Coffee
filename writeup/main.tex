...
For repeated purchases, the direction of this carryover depends on the consumption path associated with the unfinished goal. When an intended purchase follows an established routine, familiar products provide a readily accessible representation of what the consumer would otherwise have chosen. We propose that interrupting such a purchase makes this familiar path salient through counterfactual thinking and the continued accessibility of the unfinished goal. Once access returns, familiar products can therefore become more salient than unfamiliar alternatives. We refer to this response as \textit{familiar-choice salience}. This account predicts a stronger reduction in exploration among consumers whose prior purchases are more concentrated on familiar products.
...
Temporary store closures during COVID-19-related lockdowns provide the access shocks\footnote{Contemporaneous reports illustrate that local containment measures in major Chinese cities included the closure of neighborhood stores and shopping malls; see, for example, \url{https://www.xinhuanet.com/2021-01/19/c_1127001704.htm} and \url{https://m.bjnews.com.cn/detail/162795097414850.html}.}. We identify 18 closures lasting 10 to 29 days after applying the screens described in Appendix~\ref{app:closure_registry}. Our results are robust to a more relaxed selection rule leading to 22 eligible closures (Appendix Table~\ref{tab:closure_registry_sensitivity}). Panel A of Figure~\ref{fig:closure_design_timeline} illustrates the event-time construction. For each closure event $e$, we construct four pre-closure windows, indexed by $-4$ through $-1$, and four post-reopening windows, indexed by 1 through 4. Each window has the same length $L_e$ as the closure. Period 0 corresponds to the interval during which the focal store is closed. Our main analysis compares behavior before closure with behavior after reopening and excludes period 0. This construction allows outcomes to be measured over comparable windows within each event.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=\textwidth]
    {figures/coffee_closure_timeline.png}
    \caption{Event timing and the proposed mechanism}
    \label{fig:closure_design_timeline}
\end{figure}

We assign treatment exposure through consumers' pre-closure store relationships. A consumer can purchase at multiple stores, so a store closure is most likely to represent a meaningful access shock when the closing store was central to the consumer's prior purchases. We therefore restrict attention to consumers with at least five pre-closure purchase days and define a consumer's focal store as the store accounting for at least 80\% of her pre-closure purchases. Treated consumers have a focal store that closes; control consumers have a focal store among five matched stores that remain open. For each closing store, we select without replacement the five eligible control stores with the closest setup dates. Because the firm opened stores in rollout cohorts, setup timing proxies for stores' operational status and strategic importance. We exclude university and college stores because campus lockdowns and summer departures sent many students outside the focal city, where their purchases are not observed; zero local transactions could therefore reflect population departure rather than a supply interruption. Treatment therefore captures exposure to a localized access restriction, while subsequent outcomes continue to include purchases at any store in the observed chain. The resulting sample contains 40,148 consumers, including 7,390 treated and 32,758 control consumers.

\subsection{Identifying Likely Purchase Interruption}
\label{section:classifier}

Panel B of Figure~\ref{fig:closure_design_timeline} illustrates why the same closure can have different consequences for consumers. A closure is more likely to interrupt an intended purchase for a consumer who would otherwise have purchased during period 0. Under the proposed mechanism, an interrupted purchase associated with an established routine can make the familiar purchase path salient through counterfactual thinking and the continued accessibility of the unfinished goal. This representation can carry into product choices after reopening. Consumers who were less likely to purchase during period 0 are correspondingly less likely to experience this interruption. The high- and low-intention labels in the figure refer to these differences in predicted purchase propensity, which we construct using only pre-closure information.
...
The event construction yields 321,184 consumer-period observations for purchase activity. Novelty-seeking is defined only in windows containing at least one purchase. There are 105,854 such purchasing windows; after the consumer fixed-effects estimator removes consumers observed in only one product-choice period, the main novelty-seeking regressions use 99,644 observations. The classifier places 2,041 treated consumers, or 27.6\%, in the high-predicted-intention group.
...
\midrule
Consumers & 23,934 & 5,349 & 8,824 & 2,041 \\
...
\fnote{Notes: The figure plots consumer-weighted pooled means for the 18-closure analysis sample; each relative period contains 32,758 control and 7,390 treated consumers. Bars are 95\% confidence intervals. Relative period 0 is the closure window. Purchase frequency is the number of days divided by the number of calendar days in the event-length window.}
\end{figure}
...
\fnote{Notes: The figure plots the share of consumer-period observations containing at least one purchase. This is also the share of windows in which novelty-seeking is defined. Each relative period contains 23,934 control-low, 5,349 treated-low, 8,824 control-high and 2,041 treated-high consumers. Bars are 95\% confidence intervals. Means are consumer weighted. Relative period 0 is omitted to match the main regressions.}
\end{figure}

\subsection{Triple Difference-in-Differences}
\label{section:3.3}

Figure~\ref{fig:closure_design_timeline} illustrates two dimensions of our empirical comparison: behavior before closure versus after reopening in Panel A, and high versus low predicted purchase intention in Panel B. We construct both intention groups among treated consumers and consumers associated with matched stores that remain open. Comparing treated and control consumers provides the third dimension. The resulting triple difference asks whether the treated-control change in novelty-seeking is more negative among consumers whose purchases were more likely to have been interrupted. For intention group $g\in\{L,H\}$, let $\Delta_g$ denote the treated-control difference-in-differences:
\begin{equation}
\Delta_g
=
...
\label{eq:group_did}
\end{equation}
$\Delta_L$ measures the treated-control change in novelty-seeking among consumers with low predicted purchase intention, and $\Delta_H$ measures the corresponding change among consumers with high predicted purchase intention. Both groups experience closure exposure, while the likelihood that an intended purchase is interrupted is higher in the high-intention group. The triple difference compares these two responses:
\begin{equation}
DDD=\Delta_H-\Delta_L.
\label{eq:ddd_intuition}
\end{equation}
Under the identifying conditions described below, the third difference removes the component of the closure response shared across intention groups and identifies the incremental response associated with a higher likelihood of purchase interruption. This logic follows the canonical triple-differences design \citep{Gruber1994} and is closely related to recent work using temporary disruptions to distinguish exposed consumers by the likelihood that a choice opportunity was displaced \citep{Levine2026}.
...
The interpretation of $\delta^D$ as the incremental response associated with likely purchase interruption rests on two conditions. First, other consequences of closure exposure should generate similar treated-control changes among high- and low-intention consumers. These consequences may include changes in product availability, reopening promotions, shopping convenience, and consumption occasions. The third difference removes these components when their effects are shared across intention groups. The relevant concern is therefore a closure-related change that affects the two intention groups differently. For example, relatively greater promotion of familiar products to high-intention consumers, or lower exposure to novel products when they return, could also generate a negative novelty-seeking DDD.

Second, absent the closure, the treated-control gap for high-intention consumers should have evolved like the treated-control gap for low-intention consumers. This is the parallel-triple-trends condition. Importantly, it concerns changes in the treated-control gaps rather than equality of pre-period levels. Consumer fixed effects absorb persistent level differences; identification requires that the \textit{difference between the two treated-control gaps} was not already trending before the closure.
...
Prediction 2 provides a mechanism-linked test of familiar-choice salience. If an interrupted purchase reactivates an established consumption path, the reduction in exploration should be strongest among consumers whose prior choices are already more concentrated on familiar products. We test this implication using baseline novelty-seeking measured before the focal closure. The analysis asks whether the high-minus-low novelty-seeking DDD becomes more negative as consumers' prior choices become more concentrated on familiar products.

\subsection{Heterogeneity by Baseline Novelty-Seeking}
\label{section:Heterogeneity}

For each consumer, we calculate novelty-seeking separately in relative periods $-8$ through $-1$ and then average across periods in which the consumer purchases. Each of these periods has the same length as the associated closure event. Let $O_{ie}$ denote the number of distinct orders consumer $i$ places over these eight baseline windows. We require $O_{ie}\geq5$ so that the baseline measure is based on an established purchase history rather than one or two isolated transactions. The raw baseline novelty measure ranges from zero, when all purchased products are familiar, to one, when all distinct products purchased in each purchasing window are new relative to the consumer's earlier history.

The resulting sample contains 18,525 eligible consumers. Baseline novelty varies broadly: the mean is 0.508, the standard deviation is 0.269, the median is 0.500, and the 10th and 90th percentiles are 0.167 and 0.889. Figure~\ref{fig:baseline_novelty_distribution} shows that the heterogeneity test therefore uses variation across a wide range of exploratory behavior rather than a narrow split around a single cutoff.
...
\fnote{Notes: The figure shows ten equal-width bins of the raw consumer-level baseline novelty measure. The measure is the mean of within-period novelty-seeking over purchasing periods from relative period $-8$ through $-1$. Eligible consumers have at least five orders across these eight closure-length periods. The last bin includes one. The vertical line marks the sample mean. The sample contains 18,525 eligible consumers.}
\end{figure}

Let $N_{ie}$ denote standardized baseline novelty-seeking. We first estimate a direct baseline-novelty specification that augments the main DDD with a complete Post-by-baseline-novelty interaction hierarchy:
\begin{align}
Y^N_{iet}
={}& \delta^B(\text{Post}_{et}\times\text{Treated}_{ie})
+\beta(\text{Post}_{et}\times\text{High}_{ie})
+\delta^D(\text{Post}_{et}\times\text{Treated}_{ie}\times\text{High}_{ie}) \notag\\
&+\text{Post}_{et}\times N_{ie}
\left(\eta_0+\eta_T\text{Treated}_{ie}+\eta_H\text{High}_{ie}
+\theta^D\text{Treated}_{ie}\times\text{High}_{ie}\right) \notag\\
&+\phi_{ie}+\omega_t+\gamma_m+\varepsilon_{iet}.
\label{eq:baseline_novelty_heterogeneity}
\end{align}

The coefficient $\theta^D$ measures how the high-minus-low novelty-seeking DDD changes with a one-standard-deviation increase in baseline novelty. Prediction 2 implies $\theta^D>0$: the negative interruption response should attenuate as consumers become more exploratory.

Because novelty is defined relative to a consumer's accumulated purchase history, the measured baseline novelty share can also depend on the amount of history available. As consumers accumulate orders, more products enter their familiar set, so the same underlying willingness to explore can generate a lower measured novelty share when the observed history is longer. In the eligible sample, baseline novelty is correlated at $-0.329$ with $\log(1+O_{ie})$: mean baseline novelty is 0.620 among consumers with exactly five baseline orders and 0.365 among consumers with at least 20. We therefore report a second specification that adds an analogous complete Post-interaction hierarchy for
\[
Q_{ie}
=
...
\]
the standardized log number of baseline orders. This order-history-adjusted specification allows the DDD to vary separately with baseline novelty and with the amount of purchase history used to construct it. Because $N_{ie}$, $Q_{ie}$, treatment status, and predicted-intention status are time invariant within a consumer, their levels are absorbed by $\phi_{ie}$; only their interactions with Post are estimable.
...
\toprule
 & (1) & (2) \\
 & \shortstack{Baseline-novelty\\interaction} & \shortstack{Order-history-adjusted\\interaction} \\
\midrule
DDD at mean covariate values ($\delta^D$) & $-3.05$ & $-1.30$ \\
 & (2.01) & (2.00) \\
Change in DDD per SD of baseline novelty ($\theta^D$) & \textbf{2.29*} & \textbf{2.94**} \\
 & (1.43) & (1.47) \\
One-sided $p$-value for $\theta^D>0$ & \textbf{.0636} & \textbf{.0312} \\
Joint pretrend $p$-value for $\theta^D$ & .4094 & .5611 \\
Observations & 79,578 & 79,578 \\
Closure-event clusters & 18 & 18 \\
Consumer, relative-period, and calendar-month fixed effects & Yes & Yes \\
Post-by-baseline-order interaction hierarchy & No & Yes \\
\bottomrule
\end{tabularx}
\vspace{1mm}
\fnote{Notes: Estimates and standard errors are in percentage points. Baseline novelty-seeking is the consumer-level mean novelty over relative periods $-8$ through $-1$ and is standardized over eligible consumers. Column (1) estimates equation~\eqref{eq:baseline_novelty_heterogeneity}. Column (2) additionally includes a complete Post-by-baseline-order interaction hierarchy using standardized $\log(1+\text{baseline orders})$. The DDD is evaluated at mean baseline novelty in column (1) and at mean baseline novelty and mean baseline order history in column (2). Both specifications use the restricted heterogeneity sample. Standard errors use CRV1 clustering by the 18 closure events. * $p<0.10$, ** $p<0.05$, based on the one-sided directional test of $\theta^D>0$.}
\end{table}

Table~\ref{tab:baseline_novelty_heterogeneity} reports both specifications. Column (1) directly interacts the DDD with baseline novelty-seeking. The estimated gradient is positive at 2.29 percentage points per standard deviation (SE $=1.43$; one-sided $p=.064$). Column (2) additionally allows the post-period response in every DDD cell to vary with baseline order history. The estimated baseline-novelty gradient increases modestly to 2.94 percentage points (SE $=1.47$; one-sided $p=.031$). The two estimates have the same predicted sign and similar magnitudes, indicating that the heterogeneity pattern is present both before and after separating baseline novelty from the amount of purchase history used to construct it.

Across the two specifications, a one-standard-deviation increase in baseline novelty makes the interruption-related DDD 2.29 to 2.94 percentage points less negative. The decline in exploration is therefore concentrated among consumers whose pre-closure choices were more heavily oriented toward familiar products, as predicted by familiar-choice salience. The corresponding event-study tests do not reject a differential pretrend in either specification ($p=.409$ and $p=.561$). Appendix~\ref{app:baseline_novelty_dynamics} reports the dynamic estimates for the order-history-adjusted specification.
...
The direction is informative. Treated high-intention consumers are targeted with \textit{more}, not fewer, new-product campaigns relative to the DDD comparison group. Although the records do not reveal whether consumers viewed these messages, the observed targeting differential runs in the opposite direction from that required by the notification-based explanation. If new-product notifications increase awareness or trial, this differential exposure would tend to raise their exploration and therefore work against the negative novelty DDD. The targeting records therefore do not support a relative reduction in new-product campaign exposure as an explanation for the negative novelty-seeking DDD. Appendix~\ref{app:new_product_notifications} reports the campaign construction, alternative exposure measures, and complete estimates.
...
The high-minus-low timing-induced difference in the novel-product share is $-0.10$ percentage points (SE $=0.06$; 95\% CI: $-0.22$ to $+0.02$). The corresponding difference in the number of novel products is $-0.28$ products (SE $=0.38$; 95\% CI: $-1.09$ to $+0.54$). Thus, although realized assortments evolve and high-intention consumers return earlier, their return timing generates only a small difference in measured novel-product opportunity. These results provide little support for a substantial return-timing-driven gap in realized assortment exposure among consumers returning to the focal store within 28 days. Appendix~\ref{app:reopening_assortment} reports the weekly assortment paths, construction details, and matched-store comparisons.
...
Across all three product scopes, the estimated DDD remains negative. For all non-coffee products, the estimate is $-4.13$ percentage points (SE $=3.40$; restricted wild-cluster $p=0.225$), remarkably close in magnitude to the $-4.15$-point main result. Restricting the outcome to non-coffee consumables produces an estimate of $-4.11$ percentage points (SE $=3.39$; restricted wild-cluster $p=0.230$). Restricting the sample further to non-coffee beverages produces a larger estimate of $-6.26$ percentage points (SE $=4.12$; restricted wild-cluster $p=0.119$). The three point estimates have the same sign as the all-product estimate, although each is imprecisely estimated.
...
This paper asks how consumers' willingness to explore changes when a purchase they likely intended to make is unexpectedly blocked. Using temporary store closures and predicted purchase intention, we find that the treated-control post-reopening change in novelty-seeking is 4.15 percentage points lower among high-intention consumers than among low-intention consumers, equivalent to 8.3\% of the pre-period mean. The result is nearly identical when exploration is measured by adoption of products newly introduced by the firm: the estimated DDD is $-4.17$ percentage points, or 28.6\% of that outcome's pre-period mean.
...
This appendix records construction details that are needed to interpret the empirical design but are too mechanical for the main text. The unit of assignment is a consumer. A consumer enters the analysis through a closure event for which the consumer's pre-closure purchase history identifies a meaningful focal store; the outcomes then aggregate that consumer's purchases across the observed chain.

\subsection{Closure Registry and Consumer Sample}
...
\end{tabularx}
\vspace{1mm}
\fnote{Notes: DDD estimates and standard errors are in percentage points. Both rows use the same member-first novelty outcome, four pre-closure and four post-reopening periods, consumer, relative-period and calendar-month fixed effects, and CRV1 standard errors clustered by closure event. The pretrend column reports the joint test that the three pre-period Treated $\times$ High predicted-intention coefficients equal zero, with period $-1$ omitted.}
\end{table}

\subsection{Outcome Timing and Variable Definitions}
\label{app:outcome_definitions}

Each closure event defines relative periods $t=-4,\ldots,4$. Every relative period has the same length as the closure, denoted $L_e$, and period $0$ is the closure window itself. The main post-reopening regressions omit period $0$ and compare four pre-closure windows with four post-reopening windows. This convention produces 321,184 consumer-period observations for purchase frequency. Consumers make at least one purchase in 105,854 of these rows, so novelty-seeking is defined in those rows. The consumer fixed effects then remove 6,210 rows belonging to consumers with only one product-choice period, leaving 99,644 observations in each main product-choice regression. Table~\ref{tab:treatment_control_balance} reports pre-period purchase probabilities across the four DDD cells. Table~\ref{tab:appendix_variable_definitions} summarizes the construction and interpretation of the variables used throughout the empirical analysis.
...
Scored sample & Treated and control consumers in the ex-ante period-$0$ panel. \\
...
Figure~\ref{fig:appendix_classifier_overlap} compares the distributions of the predicted period-$0$ score between treated and control consumers.
...
\Big]\mathbb{I}\{t=\ell\}+\varepsilon_{iet}.
\end{split}
...
\end{tabularx}
\vspace{1mm}
\fnote{Notes: Coefficients and standard errors are in percentage points. The dependent variable is novelty-seeking, defined only in windows with a purchase. The regression uses 99,644 observations and includes consumer, relative-period and calendar-month fixed effects. Period $-1$ is the omitted reference, and period 0 is excluded. Standard errors are clustered by closure event (18 clusters). The joint Wald test uses the closure-clustered covariance matrix.}
\end{table}
...
The support restriction retains 12,634 of 40,148 consumers for purchase frequency and 10,956 consumers for novelty-seeking before the product-choice singleton restriction. As Table~\ref{tab:appendix_common_support} shows, the purchase-frequency DDD changes sign and is imprecise ($p=0.156$); its matched-sample DDD pretrend also does not reject at 5\% but is borderline ($p=0.069$). The novelty DDD is about half the full-sample magnitude and is imprecise ($p=0.295$), while its matched pretrend does not reject ($p=0.871$). The common-support exercise therefore does not provide affirmative robustness: it shows that the headline estimates, especially purchase frequency, are sensitive to restricting the comparison to closely aligned pre-period demand cells.
\end{comment}

\subsection{Dynamics of Baseline-Novelty Heterogeneity}
\label{app:baseline_novelty_dynamics}

The collapsed heterogeneity estimates in Table~\ref{tab:baseline_novelty_heterogeneity} compare all four post-reopening periods with all four pre-closure periods. Figure~\ref{fig:appendix_baseline_novelty_event_study} reports the dynamic counterpart to the order-history-adjusted specification in column (2), replacing the collapsed post indicator with relative-period interactions while retaining the same fixed effects, baseline-order adjustment and closure-event clustering. The omitted reference period is $-1$.
...
Raw 500m competitor count, consumer mean & $-3.33$ & 1.43 & [$-6.35$, $-0.31$] & .090 \\
$\log(1+\text{500m competitor count})$, consumer mean & $-3.92$ & 1.76 & [$-7.64$, $-0.20$] & .065 \\
Raw 1,500m competitor count, consumer mean & $-4.04$ & 1.64 & [$-7.51$, $-0.57$] & .038 \\
$\log(1+\text{1,500m competitor count})$, consumer mean & $-4.11$ & 1.59 & [$-7.46$, $-0.76$] & .029 \\
