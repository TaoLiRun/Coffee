...
# Modified version: uses customer-specific removed products instead of store-level
...
no_push_members <- fread("../processed_data/no_push_members.csv", encoding = "UTF-8")
...
orders_all <- fread("../processed_data/order_commodity_result_processed.csv", encoding = "UTF-8")
...
############################################################################
# Step 3: Store-Product Timeline and Customer-Specific Removed Products
############################################################################

# 3.1 Load the complete order database
cat("Creating store-product timeline...\n")
complete_orders <- fread("../processed_data/order_commodity_result_processed.csv", 
                         encoding = "UTF-8",
...
# 3.5 Create store-week panel for new products
...
# Fill missing values with 0 (for weeks with no new products)
store_panel[is.na(new_products), new_products := 0]

# 3.6 Create lagged variables for new_products
cat("Creating lagged variables for new products...\n")
...
# Apply to new_products only (removed products will be customer-specific)
store_panel <- create_lagged_variables(store_panel, "new_products")

# 3.7 Calculate customer-specific removed products
cat("Calculating customer-specific removed products...\n")

# Get all products each customer has purchased in each store (up to each week)
# For each customer-store-product, find the first week they purchased it
customer_product_first_purchase <- orders_all[, list(
  first_purchase_week = min(week_id)
), by = list(customer_id, dept_id, product_id)]

# Get product removal weeks (last_week from product_lifecycle)
product_removal_weeks <- product_lifecycle[, list(dept_id, product_id, removal_week = last_week)]

# Merge customer purchases with product removal weeks
customer_product_removals <- merge(customer_product_first_purchase, 
                                   product_removal_weeks,
                                   by = c("dept_id", "product_id"),
                                   all.x = TRUE)

# Only keep products that were purchased before they were removed
# (i.e., first_purchase_week < removal_week)
customer_product_removals <- customer_product_removals[
  first_purchase_week < removal_week | is.na(removal_week)
]

# For each customer-store-week, count how many of their previously-purchased products
# were removed in that week
# Create a panel of all customer-store-weeks
customer_store_week_panel <- unique(customer_panel[, list(customer_id, dept_id, week_id)])

# For each customer-store-product, create a record for the week it was removed
customer_removed_products <- customer_product_removals[
  !is.na(removal_week),
  list(customer_id, dept_id, week_id = removal_week)
]

# Count removed products per customer-store-week
customer_removed_count <- customer_removed_products[, list(
  removed_customer = .N
), by = list(customer_id, dept_id, week_id)]

# Merge with customer-store-week panel
customer_removed_panel <- merge(customer_store_week_panel, 
                                customer_removed_count,
                                by = c("customer_id", "dept_id", "week_id"),
                                all.x = TRUE)

# Fill missing values with 0
customer_removed_panel[is.na(removed_customer), removed_customer := 0]

# Sort for lagging
setorder(customer_removed_panel, customer_id, dept_id, week_id)

# Create lagged variables for customer-specific removed products
cat("Creating lagged variables for customer-specific removed products...\n")
customer_removed_panel <- create_lagged_variables(customer_removed_panel, "removed_customer")

# 3.8 Save product lifecycle information
cat("Saving product lifecycle data...\n")
fwrite(product_lifecycle, "outputs/data/product_lifecycle_detailed_customer_removed.csv")
fwrite(store_panel, "outputs/data/store_week_panel_complete_customer_removed.csv")
fwrite(customer_removed_panel, "outputs/data/customer_removed_panel.csv")
cat("Store-product timeline and customer-specific removed products creation complete!\n")

############################################################################
# Step 4: Merge Panels and Add Product Details
############################################################################

# 4.1 Merge customer panel with store information (for new products)
...
# 4.2 Merge customer-specific removed products
cat("Merging customer-specific removed products...\n")
analysis_panel <- merge(analysis_panel, customer_removed_panel,
                       by = c("customer_id", "dept_id", "week_id"),
                       all.x = TRUE)

# Fill missing removed_customer values with 0
removed_customer_cols <- names(analysis_panel)[grepl("^removed_customer", names(analysis_panel))]
for (col in removed_customer_cols) {
  analysis_panel[is.na(get(col)), (col) := 0]
}

# 4.3 Identify recent vs existing products
...
# 4.4 Merge product counts with analysis panel
...
fwrite(customer_panel, "outputs/data/customer_week_panel_customer_removed.csv")
fwrite(analysis_panel, "outputs/data/analysis_dataset_customer_removed.csv")
fwrite(product_panel, "outputs/data/product_panel_customer_removed.csv")
...
# Model A for push=0 (using removed_customer_lag1 instead of removed_lag1)
cat("Model A - Push=0 group...\n")
model_a_push0 <- felm(orders ~ new_products_lag1 + removed_customer_lag1 | customer_id + week_id + dept_id + week_id:dept_id,
                      data = analysis_panel_push0)

# Model A for push=1
cat("Model A - Push=1 group...\n")
model_a_push1 <- felm(orders ~ new_products_lag1 + removed_customer_lag1 | customer_id + week_id + dept_id + week_id:dept_id,
...
model_a_with_pushes <- felm(orders ~ new_products_lag1 + removed_customer_lag1 + total_pushes | 
...
if (length(push_vars) > 0) {
  ...
  if (length(available_trigger_vars) > 0) {
    ...
    formula_str <- paste("orders ~ new_products_lag1 + removed_customer_lag1 +", push_formula, 
                        "| customer_id + week_id + dept_id + week_id:dept_id")
...
model_b_thr4_push0 <- felm(orders ~ new_products_lag1 + removed_customer_lag1 + sleep_gt_4 + 
                            new_products:sleep_gt_4 + removed_customer:sleep_gt_4 + total_pushes | 
                            customer_id + week_id + dept_id + week_id:dept_id,
                            data = analysis_panel_push0)

# Model_B_threshold_4 for push=1
cat("Model_B_threshold_4 - Push=1 group...\n")
model_b_thr4_push1 <- felm(orders ~ new_products_lag1 + removed_customer_lag1 + sleep_gt_4 + 
                            new_products:sleep_gt_4 + removed_customer:sleep_gt_4 + total_pushes | 
                            customer_id + week_id + dept_id + week_id:dept_id,
                            data = analysis_panel_push1)
...
model_c_push0 <- felm(purchases_existing ~ new_products_lag1 + removed_customer_lag1 + total_pushes | 
                      customer_id + week_id + dept_id + week_id:dept_id,
                      data = product_panel_push0)

# Model_C for push=1
cat("Model_C - Push=1 group...\n")
model_c_push1 <- felm(purchases_existing ~ new_products_lag1 + removed_customer_lag1 + total_pushes | 
                      customer_id + week_id + dept_id + week_id:dept_id,
...
model_a_dist_push0 <- felm(orders ~ new_products_lag1 + new_products_lag2 + new_products_lag3 + 
                            removed_customer_lag1 + removed_customer_lag2 + removed_customer_lag3 + total_pushes | 
                            customer_id + week_id + dept_id + week_id:dept_id,
                            data = analysis_panel_push0)
...
# 6.8 Create coefficient comparison table
cat("\nCreating coefficient comparison table...\n")

# Key variables to compare (updated to use removed_customer_lag*)
key_vars <- c("new_products_lag1", "removed_customer_lag1", "total_pushes",
              "sleep_gt_4", "new_products:sleep_gt_4", "removed_customer:sleep_gt_4",
              "new_products_lag2", "new_products_lag3",
              "removed_customer_lag2", "removed_customer_lag3")
...
fwrite(coef_comparison_wide, "outputs/results/coefficient_comparison_customer_removed.csv")
fwrite(all_results, "outputs/results/regression_results_all_customer_removed.csv")
...
# Customer-specific removed products statistics
cat("\n--- Customer-Specific Removed Products Statistics ---\n")
cat(sprintf("Average weekly customer-removed products (push=0): %.2f\n", mean(analysis_panel_push0$removed_customer, na.rm=TRUE)))
cat(sprintf("Average weekly customer-removed products (push=1): %.2f\n", mean(analysis_panel_push1$removed_customer, na.rm=TRUE)))
